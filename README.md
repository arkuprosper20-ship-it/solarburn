...
### 5. Fullscreen 3D Inspector for Every Rendered Object

Both 3D views can be enlarged to a dedicated fullscreen canvas so fine structure is readable on large displays:

* **Open the inspector**: press `⛶ EXPAND` in the card header, or double-click the canvas. `ESC` (or a double-click inside the inspector, or a click on the dimmed backdrop) closes it.
* **Zoom**: mouse wheel, two-finger pinch, the `+` / `−` buttons, the level slider, or the keyboard `+` / `−`. Range is `60%` to `400%`, and `0` resets to `100%`.
* **Pan / rotate**: drag the canvas. The solar disk pans, the Earth/globe rotates, and both stay in sync with the inline card view.
* **Object tracking**: clicking a satellite dot in the inspector selects it, recentres the camera on it and enables `🎯 TRACK`. Tracked objects keep their identity while the globe rotates, and the side panel updates with orbit, altitude and sub-satellite point.
* **Inline zoom without leaving the dashboard**: the same `+` / `−` / `⟲` controls and wheel/pinch gestures work directly on the solar-disk and satellite-map cards.
